...
class MiniCourt():
    ...
    def set_canvas_background_box_position(self, frame):
        ...
        # Calculate positions for the leftmost center
        self.start_x = self.buffer # Adjusted to leftmost
        self.start_y = frame.shape[0] // 2 - self.drawing_rectangle_height // 2 # Center vertically
        self.end_x = self.start_x + self.drawing_rectangle_width
        self.end_y = self.start_y + self.drawing_rectangle_height

    def draw_court(self, frame):
        # Smoother drawing by adjusting the thickness
        for i in range(0, len(self.drawing_key_points), 2):
            x = int(self.drawing_key_points[i])
            y = int(self.drawing_key_points[i + 1])
            cv2.circle(frame, (x, y), 5, (0, 0, 255), -1)

        # Draw Lines with thicker lines for smoother appearance
        for line in self.lines:
            start_point = (int(self.drawing_key_points[line[0] * 2]), int(self.drawing_key_points[line[0] * 2 + 1]))
            end_point = (int(self.drawing_key_points[line[1] * 2]), int(self.drawing_key_points[line[1] * 2 + 1]))
            cv2.line(frame, start_point, end_point, (255, 255, 255), 3) # Increased thickness from 2 to 3, color changed to white for a cleaner look

        # Draw net with a different color for contrast
        net_start_point = (self.drawing_key_points[0], int((self.drawing_key_points[1] + self.drawing_key_points[5]) / 2))
        net_end_point = (self.drawing_key_points[2], int((self.drawing_key_points[1] + self.drawing_key_points[5]) / 2))
        cv2.line(frame, net_start_point, net_end_point, (255, 0, 0), 3) # Changed color to red for contrast, increased thickness

        return frame

    def draw_background_rectangle(self, frame):
        shapes = np.zeros_like(frame, np.uint8)
        cv2.rectangle(shapes, (self.start_x, self.start_y), (self.end_x, self.end_y), (0, 0, 0), cv2.FILLED)
        out = frame.copy()
        alpha = 0.5
